...
const ContenedorCampoFormulario = styled.div`
  ...
  padding-bottom: 1rem;
  margin-top: 1rem;
  .SeleccionaColorGénero {
    width: 50%;
    height: 2rem;
  }
  .SeleccionaColorGénero:hover {
    cursor: pointer;
  }
`;
...
export const ListaOpciones = (props) => {
  const { titulo, generos, valor, setCategoria } = props;
    ...
    <>
      <ContenedorListaOpciones>
        ...
        >
          ...
          {generos.map((musica, id) => {
            return <option key={id}>{musica}</option>;
          })}
        </select>
      </ContenedorListaOpciones>
    </>
